...
| 판정 복구가 붙으면 | **약 9,200** (실측 9,239, 판정 2회) |
| 견적 범위 | 5,774 ~ 7,176 + 복구 시 +2,520 |
...
| 설정 | 결과 |
|---|---|
| **입장 지정** | 반박이 실제로 작동. 서로 다른 근거로 같은 쟁점에서 충돌하고, Judge 의 `dissent` 가 실제 일어난 일을 요약 |
| **입장 미지정** | R1 에서 같은 편으로 **수렴**하거나, R2 에서 자기 R1 주장을 스스로 반박하며 **입장을 뒤집음** |

입장을 지정하면 `일관성` 점수가 10/10 으로 나옵니다. 이 값이 낮으면 라운드마다
입장이 바뀌고 있다는 신호입니다.
...
쟁점 3~5개 추출 · 발언마다 finish_reason='stop' · 일관성 8~10
```
