...
class ContextPack:
    ...
    def render(self) -> tuple[Message, ...]:
        ...
        if self.round_no == 1:
            parts.append(
                "[당신의 차례] 제1라운드 입론을 작성하십시오.\n"
                "- 당신의 입장을 분명히 밝히고, 가장 강한 근거부터 제시하십시오.\n"
                "- 반대편이 제기할 가장 유력한 반론 하나를 스스로 짚고 답하십시오."
            )
        else:
            # R2 이후가 "발언을 작성하십시오"뿐이면, 직전 라운드 전문은 반박
            # 대상이 아니라 **재료**로 읽힙니다. 같은 모델·같은 페르소나면
            # 상대 근거를 그대로 이어받아 강화하는 게 자연스러운 이어쓰기이고,
            # 실제로 그렇게 수렴했습니다. 라운드의 목적을 명시해야 합니다.
            parts.append(
                f"[당신의 차례] 제{self.round_no}라운드입니다. "
                "이 라운드의 목적은 **반박**입니다.\n"
                "- 직전 라운드에서 **당신과 생각이 다른 주장 하나를 골라 지목**하고, "
                "어디가 왜 약한지 구체적으로 반박하십시오. "
                "(무엇을 주장했는지 → 어디가 문제인지 → 근거)\n"
                "- 상대가 든 근거·사례·인용을 **그대로 가져다 자기 주장을 강화하지 "
                "마십시오.** 같은 근거를 쓰려면 상대의 해석이 왜 틀렸는지 먼저 "
                "밝히십시오.\n"
                "- 반박 없이 새 주장만 덧붙이는 발언은 이 라운드의 목적에 맞지 "
                "않습니다.\n"
                "- 정말로 동의한다면 동의한다고 **명시**하고, 그래도 남는 쟁점이 "
                "무엇인지 드러내십시오. 동의를 숨기고 같은 말을 반복하지 마십시오.\n"
                "- **제1라운드에서 취한 입장을 유지하십시오.** 생각이 바뀌었다면 "
                "무엇이 당신을 설득했는지 명시하고 바꾸십시오. 말없이 반대편으로 "
                "넘어가면 토론이 아니라 독백 교대가 됩니다. 상대를 반박하는 "
                "형식만 갖추고 자기 주장을 뒤집는 것도 같습니다."
            )
...
# ── 프롬프트 조립 ────────────────────────────────────────────────────────────
# ContextPack.render() 와 같은 층에 둡니다. 비용 견적기가 헤더 길이를 실제로
# 재려면 이걸 읽어야 하는데, agent.py 에 두면 cost -> agent -> provider -> cost
# 순환이 생깁니다. 의존 방향은 한쪽이어야 합니다.
